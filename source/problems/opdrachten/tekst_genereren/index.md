...
Je kan een paar voorbeelden vinden van hoe je in Python dictionary's kan gebruiken om tekst te analyseren met een *vocabulaireteller* (in tegenstelling tot een woordenteller) in {download}`deze voorbeeldcode; dit kan je gebruiken als naslagwerk voor wat je nodig hebt... </problems/assets/file_and_dictionary_examples.py>`

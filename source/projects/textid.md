...
Je kan hier voorbouwen op de code die je eerder hebt geschreven voor de [Markov-opgave](/problems/opdrachten/tekst_genereren/index) om het einde van zinnen te herkennen.
...
* Of {download}`deze stemmer <assets/snowball.py>` gebaseerd op de Nederlandse stemmer in *Snowball*, een bibliotheek van stemmers. Deze is ook niet perfect, maar een redelijke kandidaat om te gebruiken.
